...
int addOK(int x, int y) { //
  // focus, all signed int plz... not unsigned god
  // if they have diff signs, the sum can not get overflow
  // if the most significant bits of them are same, check if the MSB of the sum is the same as either msb of x or msb of y
  int sum=x+y;
  int xMSB=x>>31;
  int yMSB=y>>31;
  int sumMSB=sum>>31;
  // 2. 和的最高位必须和x与y的最高位都是一致的！
  // 后半句是比较全部一致的位运算小模版
  return !!((xMSB^yMSB)|(!(xMSB^sumMSB)&!(yMSB^sumMSB)));
}
/* 
...
int allEvenBits(int x) {
  // 先提取出偶数为的bit，异或0x55555555，如果都为1，那么就会变为0.
  // 用异或解决 all 问题
 ...
 *   Rating: 2
 */
int anyEvenBit(int x) { // 居然让用逻辑取反!的，惊了
  return !!(0x55555555 & x); // 两次!!即判断是否等于0，因为如果不是0 就会变为false(0)
                            // C语言中，所有 非0值 都表示真
}
/* 
...
int bitCount(int x) {
    ...
    // 超难，我们需要用二分的思想，先两位两位地统计1的个数，并将答案存在这两位上。然后四位四位地统计，接着是八位，直到统计完三十二位。 
    // 统计的时候运用错位相加的方法，需要注意的是在移位之前要将无关位置0，以免无关位相加对结果造成影响。
    x = (x & mask1) + ((x >> 1) & mask1);
    x = (x & mask2) + ((x >> 2) & mask2);
    x = (x & mask3) + ((x >> 4) & mask3);
    x = x + (x >> 8); // 这之后已经不会溢出了 就不用mask了
    x = x + (x >> 16);
 ...
 *   Rating: 1
 */
int bitMatch(int x, int y) { // 请注意读题...
  // return x & y;
  return ~(~(x&y)&~((~x)&(~y)));
}
/* 
...
int bitParity(int x) {
  // 这个可太酷了！
  // 不需要保留高位的数字，最后也用不到，所以不用处理。
  // 注意奇偶的特性，奇奇得偶，所以用亦或就非常合适！
  x ^= x >> 16;
  x ^= x >> 8;
  x ^= x >> 4;
  x ^= x >> 2;
